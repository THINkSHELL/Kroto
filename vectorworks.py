...
def matmul(A, B):
    if type(A) != list and type(B) != list:
        ...
        if type(B[0]) == list:
            res = [[A * B[i][j] for j in range(len(B[0]))] for i in range(len(B))]
        else:
        ...
        res = [sum(A[i][k]*B[k] for k in range(len(B))) for i in range(len(A))]
    elif type(B[0]) == list:
        res = [sum(A[k]*B[k][j] for k in range(len(B))) for j in range(len(B[0]))]
    else:
        res = dotproduct(A, B)
    return res

def matkronproduct(A, B):
